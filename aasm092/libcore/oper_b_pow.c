...
static int		expr_eval_b_pow(struct expr_s	**node)
{
  ...
  if (old->right->flag & old->left->flag & EXPR_TYPE_NUM)
    {
      ...
      if (num_pow(&old->left->val.num,
		  &old->right->val.num,
		  &res))
	error(ERRTYP_ERROR, "exponentiation overflow", old->location);
      *node = old->right;
...
const struct operator_s	oper_b_pow =
{
  &expr_eval_b_pow,
  OPER_BINARY,
  250,
