...
static int	binary_reloc_reduce (struct reloc_s		*rel,
				     struct expr_s		*expr,
				     const struct object_s	*obj)
{
  switch (expr->flag & EXPR_TYPE_MASK)
	...
	switch (expr->val.sym->flag & SYMBOL_TYPE_MASK)
	  ...
	  case (SYMBOL_TYPE_EXTERN):
	    error_submsg(error(ERRTYP_ERROR,
			       "binary: can't reduce `%s' external symbol relocation",
			       (*rel->expr)->location), sym->name);
	    return (0);
	    
	  default:
	    error(ERRTYP_ERROR, "binary: can't reduce symbol relocation to value",
		  (*rel->expr)->location);
	    return (0);
	  }
	...
	if ((rel->flag & RELOC_ADDEND) && (rel->flag & RELOC_ADDEND_VALUE))
	  {
	    num_init_uint(&expr->val.num, expr->val.sec->address + rel->addend);
	    rel->flag ^= RELOC_ADDEND_VALUE;
	  }
	else
	  num_init_uint(&expr->val.num, expr->val.sec->address);

	expr->flag ^= EXPR_TYPE_SECTION | EXPR_TYPE_NUM;
      }
      return (-1);		/* reduce done */

    case (EXPR_TYPE_OPERATION):
      return (((expr->val.op->flag & OPER_BINARY)
	       && binary_reloc_reduce(rel, expr->left, obj))
	      | binary_reloc_reduce(rel, expr->right, obj));
...
void			binary_reloc (const struct object_s	*obj)
{
  ...
  for (sec = obj->section_first; sec; sec = sec->next)
      ...
      for (rel = &sec->reloc; (r = *rel); )
	{
	  ...
	  if (!binary_reloc_reduce(r, *r->expr, obj))
	    error(ERRTYP_ERROR, "binary: can't reduce relocation",
		  (*r->expr)->location);
	}
    }
